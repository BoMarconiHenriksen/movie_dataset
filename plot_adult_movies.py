...
def plotting_adult_and_non_adult_movies(data):
    ...
    true_values.loc['release_date'] = pd.to_datetime(
        true_values['release_date'], errors='coerce').copy()
    # print(true_values.info())
    # print(true_values['release_date'])

    # Convert strings to Date for non-adult movies
    false_values.loc['release_date'] = pd.to_datetime(
        false_values['release_date'], errors='coerce').copy()
    ...
    false_values_count_dates.plot()

    plt.show()

    ax = false_values_count_dates.plot()
    fig = ax.get_figure()
